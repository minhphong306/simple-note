<template>
  <div class="ocg-header" :style="style">
    <div
      class="ocg-header--logo"
      :style="{ visibility: !style.isHiddenLogo ? null : 'hidden' }"
    >
      <img alt="OCG" src="@/assets/logo.png" width="54" height="54" />
      <h1>OCG</h1>
    </div>
    <div class="ocg-header--store-info">
      <ul>
        <li :class="{ 'is-offline': isOffline }">
          <span class="ocg-header--label">ShopBase</span>
          <span class="ocg-header--value">{{
            isOffline ? "Offline" : "Online"
          }}</span>
        </li>
        <li v-if="isRefresh" class="is-offline">
          <span class="ocg-header--label">Refresh browser</span>
          <span class="ocg-header--value">
            <svg
...
export default {
  ...
  methods: {
    async fetchHolidayStyle() {
      ...
      fetch(
        "https://a0a0a62f-a373-4f0a-bd2a-1dea0a596890.mock.pstmn.io/ocg-devtool/style-holiday.json"
      )
        .then(r => r.json())
        .then(result => {
          if (result.success) {
